...
import com.bazl.dna.database.transfer.model.bo.ExternalSysInfo;
...
import com.bazl.dna.database.transfer.task.service.SubmitToNation2CriminalService;
    ...
    private void versionTransfer(){
        if(!TaskStaticFlag.TRANSFER_CASE_RUUNING) {
            ...
            try {
                ...
                transferCaseQueueList = transferCaseQueueService.selectPendingList(transferNationParamsConfig.getLabServerNo());
    ...
    private void updateTransferCaseQueue(TransferCaseQueue transferCaseQueue, Map<String, String> transferMap) throws Exception {
        try {
            ...
            if (transferResult.equals("-1")) {
                ...
                transferCaseQueue.setTransferFileRootPath(fileRootPath);
...
//                transferMap = submitToNation2PersonService.submitToNation(submitNationDataModel);
    ...
    private SubmitNationDataModel getSubmitNationDataModel(TransferCaseQueue transferCaseQueue){
        ...
        String caseId = transferCaseQueue.getCaseId();
        ...
        if(ListUtils.isNotEmptyList(queueSampleInfoModelList)){
            sampleGeneInfoModelList = getSampleGeneInfoModelList(caseInfoModel, queueId);
        }
        submitNationDataModel.setSampleGeneInfoModelList(sampleGeneInfoModelList);
    ...
    private CaseInfoModel getCaseInfoModel(String caseId){
        ...
        if(StringUtils.isNotBlank(caseInfoModel.getCaseType())){
            if(TransferHelper.isCaseTypeCriminal(caseInfoModel.getCaseType())){
                caseInfoModel.setCaseType(caseInfoConfigs.getCaseTypeCriminal());
            }else if(TransferHelper.isCaseTypeNotCriminal(caseInfoModel.getCaseType())){
                caseInfoModel.setCaseType(caseInfoConfigs.getCaseTypeNotCriminal());
            }else if(TransferHelper.isCaseTypeCivil(caseInfoModel.getCaseType())){
        ...
        if(StringUtils.isNotBlank(caseInfoModel.getCaseProperty())){
            ...
            casePropertyMap = nationDataConverterMap.getCasePropertyMap();
            ...
            if(casePropertyMap != null
                ...
                caseInfoModel.setCaseProperty(TransferHelper.convertToNationCaseProperty(caseInfoModel.getCaseProperty(), caseInfoConfigs));
            }
        }else{
        ...
        if(StringUtils.isNotBlank(caseInfoModel.getCaseStatus())){
            caseInfoModel.setCaseStatus(TransferHelper.convertToNationCaseStatus(caseInfoModel.getCaseStatus(), caseInfoConfigs));
        }else{
        ...
        if(StringUtils.isNotBlank(caseInfoModel.getCaseLevel())){
            caseInfoModel.setCaseLevel(TransferHelper.convertToNationCaseLevel(caseInfoModel.getCaseLevel(), caseInfoConfigs));
        }else{
    ...
    private ConsignmentModel getConsignmentModel(String caseId){
        ...
        consignmentModel.setId(consignmentModel.getId());
        ...
        if(isMissingPeopleConsignment){
            ...
            String consignmentType = TransferHelper.convertToNationConsignmentType(consignmentModel.getCategory(), caseInfoConfigs);
    ...
    private List<PhysicalEvidenceModel> getPhysicalEvidenceModelList(CaseInfoModel caseInfoModel, List<PhysicalEvidenceModel> physicalEvidenceModelList){
        ...
        for(PhysicalEvidenceModel physicalEvidenceModel : physicalEvidenceModelList){
            ...
            if(StringUtils.isNotBlank(physicalEvidenceModel.getPhysicalEvidenceType())){
                physicalEvidenceModel.setPhysicalEvidenceType(TransferHelper.convertEvidenceCarrierType(physicalEvidenceModel.getPhysicalEvidenceType(), caseInfoConfigs));
            }else{
    ...
    private List<SampleInfoModel> getSuspectPersonSampleList(List<SampleInfoModel> sampleInfoModelList){
        ...
        for(SampleInfoModel sampleInfoModel : sampleInfoModelList) {
            ...
            if(TransferConstants.INSTORE_SAMPLE_TYPE_OFFENDER.equals(String.valueOf(instoreSampleInfo.getSampletype()))
                    || TransferConstants.INSTORE_SAMPLE_TYPE_SUSPECT.equals(String.valueOf(instoreSampleInfo.getSampletype()))) {
    ...
    private List<SampleInfoModel> getSampleInfoModelList(CaseInfoModel caseInfoModel, List<SampleInfoModel> sampleInfoModelList){
        ...
        for(SampleInfoModel sampleInfoModel : sampleInfoModelList){
            ...
            if(TransferHelper.isInstoreSampleTypeEvidence(instoreSampleType, sampleInfoModel.getEvidenceNo())){
                ...
                //亲缘样本处理
                if(TransferConstants.INSTORE_SAMPLE_TYPE_SUSPECT_RELATIVE.equals(instoreSampleType)
    ...
    private SampleInfoModel setRelationsInfo(CaseInfoModel caseInfoModel, SampleInfoModel sampleInfoModel){
        ...
        if (ListUtils.isNotEmptyList(relativeSampleInfoList)) {
            ...
            for (RelativeSampleInfo rsi : relativeSampleInfoList) {
                //根据目标id查找另一个角色亲属关系
                List<RelativeSampleInfo> targetSampleInfoList = sampleInfoService.selectRelativeSampleByTargetPersonId(rsi.getTargetPersonId());
                if (ListUtils.isNotEmptyList(targetSampleInfoList)) {
                    for (RelativeSampleInfo target : targetSampleInfoList) {
                        //排除自身信息
                        if (StringUtils.isNotBlank(rsi.getRelationTypeCode()) &&
                                rsi.getRelationTypeCode().equals(target.getRelationTypeCode())) {
                            continue;
                        }
                        rsi.setRelationType(getRelationTypeByRole(rsi.getRelationTypeCode(), target.getSamplebRole()));
                        //第一个亲属样本编号
                        rsi.setSampleAid(rsi.getSampleNo());
                        //第二个亲属样本编号
                        rsi.setSampleBid(target.getSampleNo());
                    }
                }
            }
        }
    ...
    private int getRelationTypeByRole(String sampleaRole, String samplebRole) {
        ...
        if (TransferConstants.RELATION_TYPE_01.equals(sampleaRole) || TransferConstants.RELATION_TYPE_03.equals(sampleaRole)) {
            if (TransferConstants.RELATION_TYPE_07.equals(samplebRole) || TransferConstants.RELATION_TYPE_08.equals(samplebRole)) {
        ...
        }else if (TransferConstants.RELATION_TYPE_07.equals(sampleaRole) || TransferConstants.RELATION_TYPE_08.equals(sampleaRole)) {
            if (TransferConstants.RELATION_TYPE_01.equals(samplebRole) || TransferConstants.RELATION_TYPE_03.equals(samplebRole)) {
                relationType = TransferConstants.FIND_SPOUSE;
            }
            if (TransferConstants.RELATION_TYPE_02.equals(samplebRole) || TransferConstants.RELATION_TYPE_04.equals(samplebRole)) {
     ...
     * @return
     */
    private List<SampleGeneInfoModel> getSampleGeneInfoModelList(CaseInfoModel caseInfoModel, String queueId){

        List<SampleGeneInfoModel> sampleGeneInfoModelList = sampleInfoService.getSampleGeneListByQueueId(queueId);
    ...
    private SampleGeneInfoModel convertStrGeneInfoModel(StrDnaLibView strDnaLibView, List<LimsMarkernames> limsMarkernamesList){
        ...
        sampleGeneInfoModel.setReagentKit(strDnaLibView.getPanelName());
