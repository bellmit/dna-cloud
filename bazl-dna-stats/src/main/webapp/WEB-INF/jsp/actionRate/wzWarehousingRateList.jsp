    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <form id="consignationForm" action="<%=path%>/actionRate/wzWarehousingRateList"
                    ...
                    <div class="row">
                        <div class="col-md-3" style="padding: 0;">
                            <div class="form-group">
                                <label>送检单位</label>
                                <select class="form-control" name="delegateOrg">
                                    <option value="">请选择单位</option>
                                    <c:forEach items="${orgInfoList}" var="orgInfo" varStatus="cs">
                                        <option value="${orgInfo.orgCode}"
                                                <c:if test="${orgInfo.orgCode eq query.delegateOrg}">selected</c:if> >${orgInfo.orgName}</option>
                                    </c:forEach>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-3" style="padding: 0;">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control form_datetime" readonly="readonly"
                                       name="delegateAtStart"
                                       value="${query.delegateAtStart}"
                                       placeholder="请选择开始时间">
                            </div>
                        </div>
                        <div class="col-md-3" style="padding: 0;">
            ...
            <div class="panel-body" style="padding: 15px 0;">
                <div class="row">
                    <div class="col-md-8 sec-title">
                        <div class="title-tab " name="detectionRateBtn">
                            侵财类案件脱落细胞检出率
                        </div>
                        <div class="title-tab active" name="wzWarehousingRateBtn">
                            侵财类案件现场物证入库率
                        </div>
                    </div>
                    <div class="col-md-10 col-md-offset-10 export" name="wzWarehousingRateExpotrBtn">
                ...
                <table class="table table-hover table-bordered bigTable table-striped"
                    ...
                    <c:forEach items="${wzWarehousingRateList}" var="wzWarehousingRate" varStatus="s">
                        <tr>
                            <td>${wzWarehousingRate.delegateOrgName}</td>
                            <td>${wzWarehousingRate.comparisonCount}</td>
                            <td>${wzWarehousingRate.notComparisonRkCount}</td>
                            <td>${wzWarehousingRate.sampleCount}</td>
                            <td>${wzWarehousingRate.computedResult}</td>
                        </tr>
                    </c:forEach>
                    </tbody>
                </table>
...
<script>
    $(function () {
        ...
        /*侵财类案件脱落细胞检出率*/
        $("div[name='detectionRateBtn']").on("click", function () {
            var newUrl = "<%=path%>/actionRate/exfoliatedCellsDetectionRateList";
            $("#consignationForm").attr('action', newUrl);
            $("#consignationForm").submit();

            var oldUrl = "<%=path%>/actionRate/exfoliatedCellsDetectionRateList";
            $("#consignationForm").attr('action', oldUrl);
            //location.href = "<%=path%>/actionRate/exfoliatedCellsDetectionRateList";
        });

        /*侵财类案件现场物证入库率*/
        $("div[name='wzWarehousingRateBtn']").on("click", function () {
            location.href = "<%=path%>/actionRate/wzWarehousingRateList";
        });


        $("div[name='wzWarehousingRateExpotrBtn']").click(function () {
            var newUrl = "<%=path%>/actionRate/wzWarehousingRateExpotr";
            $("#consignationForm").attr('action', newUrl);
            $("#consignationForm").submit();

            var oldUrl = "<%=path%>/actionRate/wzWarehousingRateList";
            $("#consignationForm").attr('action', oldUrl);
