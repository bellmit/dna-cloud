 ...
 */
@RestController
@RequestMapping("/casemissing")
public class CaseMissingController extends BaseController {

	@Autowired
	private CaseMissingService caseMissingService;
	 ...
	 */
	@PostMapping("list")
	public ResponseData list(@RequestBody CaseMissingQuery query) {
		// 案件信息 委托信息列表
		Map<String, Object> result = caseMissingService.findList(query);
	...
	public ResponseData get(@PathVariable Integer id) {
		...
		List<CasePersonInfo> missingList = casePersonInfoService.listByMap(missMap);
		...
		for (CasePersonInfo casePersonInfo : missingList) {
		...
		result.put("missingRelativeList", relativeList);
	...
	public ResponseData getRelativeInfo(@PathVariable Integer id) {
		CasePersonInfo casePersonInfo = casePersonInfoService.getById(id);
		DnaSampleInfo dnaSampleInfo = dnaSampleInfoService.getOne(new QueryWrapper<DnaSampleInfo>().eq("REF_PERSON_ID", casePersonInfo.getId()));
		PersonRelativeInfo personRelativeInfo = personRelationInfoService.getOne(new QueryWrapper<PersonRelativeInfo>().eq("RELATION_PERSON_UUID", casePersonInfo.getId()));
		
		Map<String, Object> result = Maps.newHashMap();
		result.put("casePersonInfo", casePersonInfo);
		result.put("dnaSampleInfo", dnaSampleInfo);
		result.put("relationTypeCode", personRelativeInfo.getRelationTypeCode());
		return new ResponseData(result);
	}
