...
public class TokenFilter extends ZuulFilter {
	...
	public Object run() throws ZuulException {
		...
		if (StringUtils.isNotBlank(authToken)) {// 验证token
			boolean flag = JwtUtils.validateToken(authToken, secret);// 验证token是否有效
			if (flag) {
				ctx.setSendZuulResponse(true); // 对该请求进行路由
				ctx.setResponseStatusCode(HttpStatus.SC_OK); // 返回状态码
				ctx.set(IS_SUCCESS, true);
				return null;
			}
		}
		ctx.setSendZuulResponse(false); // 不对该请求进行路由
		ctx.setResponseStatusCode(HttpStatus.SC_UNAUTHORIZED); // 返回状态码
