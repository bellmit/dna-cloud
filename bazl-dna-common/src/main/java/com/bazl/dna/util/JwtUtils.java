...
public class JwtUtils {
	 ...
	 * @return
	 */
	public static String generateToken(String username, Date expird, String secret) {
		Map<String, Object> claims = Maps.newHashMap();
		claims.put("sub", username);
		claims.put("created", new Date());
		return Jwts.builder().setClaims(claims).setExpiration(expird)
				.signWith(SignatureAlgorithm.HS512, secret).compact();
	}
	...
	public static Claims getClaimsFromToken(String token, String secret) {
		...
		try {
			...
			LOGGER.error("secret:{}, token:{}", secret, token);
