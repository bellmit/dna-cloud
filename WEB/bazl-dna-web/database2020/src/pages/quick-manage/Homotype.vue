...
export default {
  ...
  mounted() {
    if (sessionStorage.getItem('panelNameSTR')) {
      ...
      console.log(this.panelNameSTR, '试剂盒')
    }
  ...
  methods: {
    ...
    submitAddMixedSTRLocus() {
      ...
      this.$axios
        .post('/database/rapid/str/submitSameCompareQueue', info)
        .then(res => {
          // console.log(res)
          sessionStorage.setItem('HomotypeResultId', res.result)
          this.$router.push({
            path: '/quickmanage/HomotypeResult'
          })
        })
    },
    searchDataBase(type) {
    ...
    changeIndexKit(obj) {
      ...
      this.$axios
        ...
        .then(res => {
          console.log(res, '查看数据0---')
          console.log(this.genelist, '查看判断')
          const list = []
    ...
    submitAddLocus() {
      ...
      // )
      console.log(this.addLocusCheckAllList)
