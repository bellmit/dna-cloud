          ...
          <Row class="item-row" :gutter="16">
            <Col class="item-col" span="24">
              ...
              <Input
                ...
                v-model="sampleNo"
              ></Input>
              <button class="btn btn-blue-bg" @click.prevent="searchDataBase('lims')">
                Lims数据库检索
              </button>
              <button class="btn btn-yel-bg" @click.prevent="searchDataBase('')">
                DNA数据库检索
              </button>
          ...
          <div class="gene-part">
            <Row :gutter="16" class="gene-row" v-show="mixPerson == 2">
              ...
              >
                ...
                <template v-for="(i, j) in item.value">
                  <Input
                    class="gene-input"
                    v-model="item.value[j]"
                    :key="i.id"
                  />
                  <span :key="i.name" v-show="j != item.value.length - 1"
                    >/</span
                  >
                </template>
                <Icon
                  @click="handaddGene(item)"
                  class="icon-add"
                  type="md-add-circle"
                  title="添加基因位点"
                  style="color:#00439E;cursor:pointer;font-size:16px;"
                />
              </Col>
            </Row>
...
export default {
  ...
  mounted() {
    if (sessionStorage.getItem('panelNameYSTR')) {
      ...
      this.panelNameYSTR = JSON.parse(arr)
    }
  ...
  methods: {
    goRecord() {
      this.$router.push({
        path: '/quickmanage/quickYstrRecord'
      })
    },
    // 添加基因座 - 弹窗 - 输入框 - 确认按钮
    handSearchGene() {
    ...
    submitAddMixedSTRLocus() {
      ...
      info.panelbId = this.panelId
      info.panelbName = this.panelName
      info.compareParams = JSON.stringify(obj)
      info.populationFrequencyId = this.populationFrequencyId
      this.$axios
        .post('/database/rapid/ystr/submitYstrCompareQueue', info)
        .then(res => {
          // console.log(res)
          sessionStorage.setItem('QucikYstrResultId', res.result)
          this.$router.push({
            path: '/quickmanage/quick_YStrResult'
          })
        })
    },
    searchDataBase(type) {
      ...
      if (this.sampleNo.length > 5) {
        this.$axios
          .get('/database/rapid/ystr/selectYstrSampleNoList', {
            params: {
              dataType: type,
    ...
    changeIndexKit(obj) {
      ...
      this.$axios
        ...
        .then(res => {
          ...
          console.log(list, '拼接数据')
          console.log(this.genelist, '基因数据')
          if (this.genelist) {
            list.forEach(item => {
              if (this.genelist.find(x => x.name === item.name)) {
                // item.value = this.genelist.find(x => x.name === item.name).value
                item.value = this.genelist.find(x => x.name === item.name).value
                // this.genelist
