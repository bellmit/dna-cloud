        ...
        <div class="result-part">
          <Row class="title">
            <Col span="12" class="left-title">
              <span>比中结果列表</span>
              <button class="btn btn-blue-bg" @click="handExportBzres">
                批量导出确认比中结果Excel
              </button>
...
export default {
  ...
  data() {
    return {
      ...
      selectByOrgInfo: [], // 委托单位数据
      clientOrgList: [] // 委托单位 v-model
    }
  },
  created() {
  ...
  methods: {
    ...
    handExportBzres() {
      if (this.checkedGroup.length > 0) {
        ...
        console.log(id_str)
        // eslint-disable-next-line camelcase
        location.href = '/api/database/export/same/result?ids=' + id_str
      } else {
        this.$Modal.error({
          title: '抱歉',
          content: '请先选择要操作的组别！'
    ...
    getStrResultQuery() {
      ...
      this.$axios
        .post('/database/match/same/resultQuery', this.searchForm)
        .then(res => {
    ...
    handleLiftConfirmAll(type) {
      if (this.checkedGroup.length > 0) {
        ...
        this.$axios
          .post('/database/match/same/updateResultCode', info)
          .then(res => {
            if (res.code === 1) {
              this.$Modal.success({
                title: '恭喜',
