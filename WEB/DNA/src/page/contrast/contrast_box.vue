<template>
  <div>
    <div class="home_title">
      ...
      <router-link tag="b" to="/break">STRMIX拆分</router-link>》
      <span>快速对比</span>
    </div>
    <div class="contrast_box">
      ...
      <div class="contrast_min">
        ...
        <div class="contrast_min_right" v-else>
          ...
          <!-- stripe  style="margin-bottom:50px;" -->
      ...
      <div class="contrast_footer" v-show="caseShow">
        <Button class="contrast_footer_btn" @click="handelSubmitConParson()">提交本案比对</Button>
...
<script>
import contrastServers from "../../servers/contrastServers";
...
export default {
              ...
              h("Icon", {
                ...
                on: {
                  click: event => {
                    ...
                    // 备用  this.modalDetail.modalType = 0;
                    // console.log(this.modalDetail.BtnType);
                    ...
                    contrastServers.LookList(Info).then(res => {
                      // 峰图路径
  ...
  methods: {
    ...
    handelLibraryComparison() {
      ...
      contrastServers.ComparisonDetails(compareContribution).then(res => {
        // console.log(res)
        if (res.code == 1) {
          this.Btnmodel = true;
        } else {
    ...
    // 本案对比
    handelSubmitConParson() {
      this.span_value = "本案比对结果";
      this.Conditions = [];
      if (this.changeValue == 0) {  // 需要改动 11.11
        this.Conditions.push(this.changeValue);
      } else {
        this.Conditions.push(this.InputVlaue);
      }
      ...
      contrastServers.dataComparison(comparisonInfo).then(res => {
        // console.log(res);
        if (res.code == 1) {
          this.LCDShow = false;
          this.caseShow = false;
          this.DisplayDOM = true;
          this.data_list = res.data.caseInfoList.dataList;
          this.allCount = res.data.caseInfoList.allRecordCount;
        } else {
          alert("参数错误,请重试");
        }
      });
