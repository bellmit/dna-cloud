<template>
    <div class="dataHome">
        <div class="home_title"><router-link tag="b" to="/">首页</router-link> 》<router-link tag="b" to="/data">数据库管理</router-link> 》 <span>比中详情</span></div>
        ...
        <div class="details_body">
            <div v-show="partOne">
              <div v-if="thanDetailsData.length != 0">
                <Table stripe border :columns="thanDetailsCol" :data="thanDetailsData" class="details_table"></Table>
                <div class="page">
                  <span>
              ...
              <div class="error-con" v-else>
                <div>
                  <Table border class="display-none"  :columns="thanDetailsCol"></Table>
            ...
            <div v-show="partTwo">
              <div v-if="thanDetailsDataTwo.length != 0">
                <Table stripe border :columns="thanDetailsColTwo" :data="thanDetailsDataTwo" class="details_table"></Table>
                <div class="page">
                  <span>
                    当前第{{currentPageTwo}}页/共{{pageCountTwo}}页/共{{allRecordCountTwo}}条信息
                  </span>
                  <!-- 此处更改page-size为动态值 10/17-->
                  <Page :total="allRecordCountTwo" show-elevator prev-text="上一页" next-text="下一页" class-name="bazl_page" :current="currentPageTwo" @on-change="changePageTwo" :page-size="pagesizeTwo"/>
                </div>
              </div>
              <div class="error-con" v-else>
                <div>
            ...
            <div v-show="partThree">
              <div v-if="thanDetailsDataThree.length != 0">
                <Table stripe border :columns="thanDetailsColThree" :data="thanDetailsDataThree" class="details_table"></Table>
                <div class="page">
                  <span>
                    当前第{{currentPageThree}}页/共{{pageCountThree}}页/共{{allRecordCountThree}}条信息
                  </span>
                  <Page :total="allRecordCountThree" show-elevator prev-text="上一页" next-text="下一页" class-name="bazl_page" :current="currentPageThree" @on-change="changePageThree" :page-size="15"/>
                </div>
              </div>
              <div class="error-con" v-else>
                <div>
    ...
    export default {
                          ...
                          h('strong', {
                            ...
                            on:{
                              click:() => {
                                ...
                                thanDetailServers.QueryMatchResultInfo(detailInfo).then(res=>{
                                    this.modalDetail.modalInfo = res.data.stringObjectMap.resultList;
        ...
        methods:{
          ...
          handleSplitSingle(){ 
            let splitSingleInfo = {
              id:this.caseId,
              curPage:this.currentPage,  // 此处改动 分页
              resultType:"04",
            }
            thanDetailServers.mixedSingleCompare(splitSingleInfo).then(res=>{
              console.log(res);
              if(res.code == 1){
                this.num_one = res.data.count1;
                this.num_two = res.data.count2;
                this.num_three = res.data.count3;
                if(res.data.matchResults.length == 0 || res.data.matchResults == undefined || res.data.matchResults == null ){
                    this.$Message.error("当前列表无数据！")
                }else{
                    this.thanDetailsData = res.data.matchResults;
          ...
          handleMixedSingle(){
            let mixedSingleInfo = {
              id:this.caseId,
              curPage:this.currentPageTwo,
              resultType:"01",
            }
            thanDetailServers.mixedSingleCompare(mixedSingleInfo).then(res=>{
              if(res.code == 1){
               this.num_one = res.data.count1;
               this.num_two = res.data.count2;
               this.num_three = res.data.count3;
                if(res.data.matchResults.length == 0 || res.data.matchResults == undefined || res.data.matchResults == null ){
                    ...
                    this.thanDetailsDataTwo = res.data.matchResults;
                    this.pageCountTwo = res.data.pageInfo.pageCount
                    this.allRecordCountTwo = res.data.pageInfo.allRecordCount
                }
              }else{
                this.$Message.error('列表获取失败！')
              }
          ...
          handleMixedSplited(){
            let mixedSplitedInfo = {
              id:this.caseId,
              curPage:this.currentPageThree,
              resultType:"02",
            }
            thanDetailServers.mixedSingleCompare(mixedSplitedInfo).then(res=>{
              console.log(res);
              if(res.code == 1){
              this.num_one = res.data.count1;
              this.num_two = res.data.count2;
              this.num_three = res.data.count3;
                if(res.data.matchResults.length == 0 || res.data.matchResults == undefined || res.data.matchResults == null ){
                    ...
                    this.thanDetailsDataThree = res.data.matchResults;
                    this.pageCountThree = res.data.pageInfo.pageCount;
                    this.allRecordCountThree = res.data.pageInfo.allRecordCount;
                }
                }else{
                  this.$Message.error('列表获取失败')
                }
