...
import com.bazl.dna.database.compare.task.queue.thread.AsyncRelativeThread;
import com.bazl.dna.database.service.service.DnaStrGeneInfoService;

@Component
public class CompareQueueRelativeTask {
	
	private static final Logger LOG = LoggerFactory.getLogger(CompareQueueRelativeTask.class);
	
	@Autowired
	private DnaStrGeneInfoService geneInfoService;
	
	@Autowired
	private AsyncRelativeThread asyncTask;
	
	@Value("${compare.queue.relative.enabled}")
