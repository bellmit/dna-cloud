 ...
 */
@Component
public class AutoCompareRelativeSingleConjoinedGeneInfoTask {
	...
	@Scheduled(initialDelay = 40000, fixedDelay = 1000 * 60 * 3)
    public void execute() {
		if (isEnabled) {
			...
			for (AutoCompareQueue compareQueue : compareQueueList) {
				compareService.sendAutoRelativeSingleConjoinedCompare(compareQueue);
