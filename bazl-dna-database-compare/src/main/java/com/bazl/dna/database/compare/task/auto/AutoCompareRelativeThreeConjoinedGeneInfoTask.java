 ...
 */
@Component
public class AutoCompareRelativeThreeConjoinedGeneInfoTask {
	...
	@Scheduled(initialDelay = 50000, fixedDelay = 1000 * 60 * 3)
    public void execute() {
		if (isEnabled) {
			...
			for (AutoCompareQueue compareQueue : compareQueueList) {
				compareService.sendAutoRelativeThreeConjoinedCompare(compareQueue);
