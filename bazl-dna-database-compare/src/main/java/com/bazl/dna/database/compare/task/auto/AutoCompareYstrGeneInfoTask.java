 ...
 */
@Component
public class AutoCompareYstrGeneInfoTask {
	...
	@Scheduled(initialDelay = 20000, fixedDelay = 1000 * 60 * 3)
    public void execute() {
		if (isEnabled) {
			// 1.获取需要比对队列的样本 compareQueue 条件: YSTR 未比对
			...
			for (AutoCompareQueue compareQueue : compareQueueList) {
				compareService.sendAutoYstrCompare(compareQueue);
