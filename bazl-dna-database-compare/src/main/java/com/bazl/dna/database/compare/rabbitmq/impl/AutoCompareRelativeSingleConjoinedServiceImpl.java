...
import com.bazl.dna.database.compare.rabbitmq.IAutoCompareRelativeSingleConjoinedService;
...
@Service
public class AutoCompareRelativeSingleConjoinedServiceImpl implements IAutoCompareRelativeSingleConjoinedService {
	
	private static final Logger LOG = LoggerFactory.getLogger(AutoCompareRelativeSingleConjoinedServiceImpl.class);
	...
	public void saveMatchResult(AutoCompareQueue compareQueue, List<AlleleFrequencyInfo> frequencyList,
		...
		try {
			LOG.info("Auto Compare Relative Single: {}", id);
			// 目标基因
			...
			for (PersonRelativeInfo targetRelationInfo : targetRelationInfoList) {
				...
				LOG.info(" 关系为：{} - 单亲的亲缘关系为：{} 亲缘样本: {}", compareRelation, relationb, sampleb);
	...
	public void compare(AutoCompareQueue compareQueue) {
		try {
			LOG.info("自动比对:开始 单亲比对 {}", compareQueue.getId());
			// 2.获取比对条件
			...
			LOG.info("自动比对:完成 单亲比对 {}", compareQueue.getId());
		} catch (Exception e) {
