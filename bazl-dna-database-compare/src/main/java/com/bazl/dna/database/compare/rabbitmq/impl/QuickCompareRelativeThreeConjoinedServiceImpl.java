...
import com.bazl.dna.database.compare.rabbitmq.IQuickCompareRelativeThreeConjoinedService;
...
@Service
public class QuickCompareRelativeThreeConjoinedServiceImpl implements IQuickCompareRelativeThreeConjoinedService {
	
	private static final Logger LOG = LoggerFactory.getLogger(QuickCompareRelativeThreeConjoinedServiceImpl.class);
	...
	public void saveMatchResult(QuickCompareQueue compareQueue, List<AlleleFrequencyInfo> frequencyList, 
		...
		try {
			LOG.info("Quick Compare Relative Three: {}", id);
			...
			String relationb = compareQueue.getPendingSamplebRelation();
	        ...
	        if (relationb != null) {
		        	...
		        	if (Constants.COMPARE_TARGET_RELATION_FATHER.equals(relationb)) {
		        		// 父
		        		fatherGeneInfoList = JSONObject.parseArray(GeneTransFormUtils.geneFormatString2(compareQueue.getPendingSamplebGeneInfo()), DnaGeneInfoDetail.class);
		        } else if (Constants.COMPARE_TARGET_RELATION_MOTHER.equals(relationb)) { 
		        		// 母
		        		motherGeneInfoList = JSONObject.parseArray(GeneTransFormUtils.geneFormatString2(compareQueue.getPendingSamplebGeneInfo()), DnaGeneInfoDetail.class);
		        } else {
		        		// 子
		        		childGeneInfoList = JSONObject.parseArray(GeneTransFormUtils.geneFormatString2(compareQueue.getPendingSamplebGeneInfo()), DnaGeneInfoDetail.class);
		        }
