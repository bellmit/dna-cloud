    ...
    <div class="introduce">
        ...
        <div>
            ...
            <div class="sumSet">
                <div>
                    <p>今年案件总数</p>
                    <p id="caseYearCount">${caseYearCount}</p>
                </div>
                <div>
                    <p>去年同期案件总数</p>
                    <p id="caseFormerYearCount">${caseFormerYearCount}</p>
                </div>
                <div>
                    <p>案件量同比</p>
                    <p id="percentage">
                        <c:choose>
                            <c:when test="${countFlag == '1'}"></c:when>
                            <c:when test="${countFlag == '2'}">
                                <span><img src="<%=path%>/img/to.png" alt="" style="transform: rotate(180deg);"></span>
                            </c:when>
                            <c:otherwise>
                                <span><img src="<%=path%>/img/to.png" alt="" ></span>
                            </c:otherwise>
                        </c:choose>
    ...
    function obtainAreaInfo(areaCode) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#caseYearCount").text(data.caseYearCount);//今年案件数
                $("#caseFormerYearCount").text(data.caseFormerYearCount);//去年案件数
                $("#percentage .percent").text(data.percentage+'%');//案件量同比
                var countFlag = data.countFlag;
                if (countFlag == '1') {
                    $("#percentage img").css("display","none")
                } else if (countFlag == '2') {
                    $("#percentage img").css("transform","rotate(180deg)")
                } else {
                    $("#percentage img").css("transform","rotate(0deg)")
                }
                ddd = data.caseSum;//案件总数
                backlogCharts2Option()
            }, error: function () {
            }
        });
    }

    function backlogCharts2Option() {
        var backlogCharts2 = echarts.init(document.getElementById('backlogCharts2'));
        var backlogCharts2Option = {
            title: {
                subtext: '单位/件',
