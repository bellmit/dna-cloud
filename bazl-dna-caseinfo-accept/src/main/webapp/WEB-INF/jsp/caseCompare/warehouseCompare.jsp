...
<div class="row btn-box">
    <div class="col-md-12">
        ...
        <button type="button" class="btn btn-blue-border btn-lang" name="returnBtn">返回</button>
    </div>
</div>
...
//                    that.parents(".col-md-3").next().find("select").val(relationType)
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.success || data.success == true || data.success == "true") {
                        ...
                        location.href="<%=path%>/center/warehouseCompare?caseId="+caseId;
                    }
                },
                error: function (data) {
                    $('#loading').modal('hide');
        ...
        $("button[name='returnBtn']").on("click", function () {
            ...
            var returnStatus = $("input[name='returnStatus']", $(this).parent()).val();

            if (returnStatus == '1'){
                //返回到案件比对页面
                location.href = "<%=path%>/center/enterCaseCompare?caseId=" + caseId +"&consignmentId="+consignmentId;
