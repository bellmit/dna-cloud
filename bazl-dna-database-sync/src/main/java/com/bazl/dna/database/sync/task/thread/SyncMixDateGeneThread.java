...
@Component
public class SyncMixDateGeneThread {

    private static final Logger LOG = LoggerFactory.getLogger(SyncMixDateGeneThread.class);
    ...
    public void execute(List<Map<String, String>> listLocusName,
                        int pageIndex, int pageSize, Date date) {
        try {
            LOG.info("Thread: {}, limit:{},{}", Thread.currentThread().getName(), pageIndex, pageSize);
            Gson gson = new Gson();
            LOG.info("------------查询近" + date + "天的单一样本-------------");
            ResponseData selectSingleGeneByCwsdPage = geneListServerClient.selectByDateSingList(String.valueOf(pageIndex),pageSize, date);
