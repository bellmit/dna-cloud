...
$(function () {
    ...
    layui.use(['layer', 'form', 'upload'], function () {
        ...
        var wsfsType = $("#wsfsType").val();
        var inContent=$("#inContent").val();
        form.on('radio(instrFilter)', function (data) {
            var type=data.value;
            if(type=="1"){
                $("input[name='instrumentsContent']").val("");
                $("#instrumentsContentDiv").hide();
                ...
                if(type!=wsfsType){
                    ...
                    $("input[name='instrumentsContent']").val(inContent);
                }
                $("#addressContent").html("邮寄地址<i class=\"icolor\">*</i>");
                $("#instrumentsContentDiv").show();
            }else if(type=="3"){
        ...
        $("#addSampleInfo").click(function () {
            ...
            var samplingType = $("#samplingType").val();
            ...
            if(flag=="add"){
                ...
                // text += '<input type="hidden" name="samplingType" value="' + $("#samplingType").find("option:selected").text() + '">';
                ...
                text += '<td>' + $("#samplingType").find("option:selected").text() + '<input type="hidden" name="samplingType" value="' + samplingType + '"></td>';
                ...
                str.find("[name='samplingType']").val($("#samplingType").find("option:selected").text());
        ...
        $("button[name='downFiles']").on("click", function () {
            window.location.href = baseurl + "/poisonCase/nextPoisonCase.html?caseId="+$("#caseId").val();
        });
       /* $("#addNew").on("click", function () {
            $('#newFiles').trigger("click");

        });*/
        //上传页面跳转
        $("button[name='newFiles']").on("click", function () {
            window.location.href = baseurl + "/poisonCase/poisonCase.html";
        });



        //保存案件信息
        $("#saveAllCaseInfo,#addNew").on("click", function () {
            var btnVal=$(this).val();
            //鉴定类别
            var a = $("input[name='project']:checked").val();
            var aText = $("input[name='project']:checked").attr("title");
            ...
            var arrText=[];
            //鉴定项目
            $("input[name='Major']:checkbox").each(function (index) {
            ...
            $("#censorship").val($("#censorshipId").find("option:selected").text());



            // if (censorship == "") {
            //     layer.msg('请输入承办人！！', {icon: 6, offset: '250px'});//icon 1:对号2：x号 3：？号 4：锁号 5：哭脸 6：笑脸
            //     return false;
            // }
            ...
            // if (contactsAddress == "") {
            //     layer.msg('请输入委托人地址！！', {icon: 6, offset: '250px'});//icon 1:对号2：x号 3：？号 4：锁号 5：哭脸 6：笑脸
            //     return false;
            // }
            ...
            var instrumentsType=$("input[name='instrumentsType']:checked").val();
            if(instrumentsType=="2" || instrumentsType=="3"){
                var instrumentsContent=$("input[name='instrumentsContent']").val();
                if(instrumentsContent==""){
                    layer.msg('请输入文书发送地址或内容！！', {icon: 6, offset: '250px'});//icon 1:对号2：x号 3：？号 4：锁号 5：哭脸 6：笑脸
                    return false;
                }
            }
           /* var appraiser2 = $("#appraiser2").val();
            ...
            $('#samplesTable tr').each(function (i) {
                ...
                var samplingType = $(this).find("[name='samplingType']").val();//采样类型
...
function initIdenProject(id) {
    var idenMajor=$("#idenMajor").val().split(",");
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data.identificationProjectList, function (index, item) {
                    text += '<input type="checkbox" onkeyup = "getMajorValue(this)" lay-skin="primary" lay-filter="identifyMajor" name="Major" value="' + item.id + '"' ;
                    $.each(idenMajor,function (index,m) {
                        if(m == item.id){
                            text += 'checked="true"';
                        }
                    })
                    text+= ' title="' + item.identificationProject + '">';
                });

                $("#jdxm").append(text);
                $("#caseContent").val($("input[name='title']").val());
...
function getMajorValue(obj){


    var dataValue = $("input[name='Major']:checked").val();
    alert(dataValue);
    $("#caseContent").val(dataValue);
}
...
function modifySample(obj){
    ...
    var personship  = str.find("[name='personship']").val();//被鉴定人
    var samplingType = str.find("[name='samplingType']").val();//采样类型
    ...
    samName=sampleName;
    $("#sampleType").val(sampleType);//检材类型
    $("#samplingType").val(samplingType);//采样类型
    $("#amount").val(amount);
