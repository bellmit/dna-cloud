    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <form id="bringConsignationForm" action="<%=path %>/center/bringWaitAccept" class="form-horizontal tasi-form"
                      method="post">
                    <div class="row">
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <table class="table table-hover table-bordered bigTable">
                    ...
                    <tbody>
                    <c:forEach items="${reCaseInfoList}" var="list" varStatus="s">
                        <tr>
                            <td>${s.index+1}</td>
                            ...
                            <td>
                                ...
                                <input type="hidden" name="consignmentId" value="${list.consignmentId}">
                                <a href="<%=path%>/center/upload?caseId=${list.entity.caseId}&consignmentId=${list.consignmentId}"
                                   target="ifm" class="btn btn-red btn-sm">受理</a>
                            </td>
                        </tr>
                    </c:forEach>
                    </tbody>
...
<script>
    $(function () {
        ...
        kkpager.generPageHtml({
            pno: ${reCasePageINfo.page},
            //总页码
            total: ${reCasePageINfo.pageCount},
            //总数据条数
            totalRecords: ${reCasePageINfo.allRecordCount},
            //链接前部
            hrefFormer: '<%=path%>/center/bringWaitAccept',
            //链接尾部
            //hrefLatter: '.html',
            getLink: function (page) {
                return this.hrefFormer + this.hrefLatter + "?" + "page=" + page + "&" + $("#bringConsignationForm").serialize();
            }
            , lang: {
                firstPageText: '首页',
                firstPageTipText: '首页',
    ...
    $("button[name='reset']").on("click", function () {
        location.href = "<%=path%>/center/bringWaitAccept";
    })


    $("#addInformant").on("click", function () {
        $("#page").val(1);
        $('#bringConsignationForm').submit();
    });
    ...
    $("input[name='resetBtn']").on("click", function () {
        $("#bringConsignationForm").find('input[type=text],select,input[type=hidden]').each(function () {
            $(this).val('');
        });
        $("input[name='visitNum']").val(0);
    });
