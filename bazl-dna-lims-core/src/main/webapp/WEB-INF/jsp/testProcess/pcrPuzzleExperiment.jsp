                            ...
                            <div class="select">
                                <select class="form-control" id="boardNo" name="boardNo">
                                    ...
                                    <c:forEach items="${pcrlist}" var="pcr">
                                        <option value="${pcr.boardNo}"
                                                <c:if test="${pcr.boardNo eq boardNo}">selected</c:if>>${pcr.boardNo}</option>
                                    </c:forEach>
                                </select>
                            </div>
                        </div>
            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-heading red" style="height: 54.6px">
                        <div style="margin-top: 5px">扩增阶段</div>
                    </div>
                                    ...
                                    <c:forEach items="${tempList.list}" var="list" varStatus="s">
                                        <li>
                                            ...
                                            || list.samplePostion eq 'E12'  || list.samplePostion eq 'F12' }">
                                                ...
                                                <c:otherwise>
                                                    <div value="${list.samplePostion}"></div>
                                                </c:otherwise>
                                            </c:choose>
                ...
                <div class="panel panel-default">
                    <div class="panel-heading blue">
                        ...
                        <button class="btn btn-blue-border checkAll" data-toggle="popover" data-placement="top"
                                data-content='选中检材放入到左侧扩增板中' data-trigger="focus">全选
                        </button>
                    </div>
                    <div class="panel-body" style="height: 570px;border-left: 1px solid #d5d5d5;padding-bottom: 0px;overflow-y: auto">
                        <div>
                            <div class="row">
                                <c:forEach items="${labPcrSampleList}" var="labPcrSample" varStatus="em">
                                    <div class="col-md-6 nopadding">
                                        <label class="custom-control checkbox-inline nopadding">
                                            <input class="custom-control-input sampleNo" type="checkbox" name="checkAll"
                                                   value="${labPcrSample.sampleNo}"
                                                   samplePostion="" sampleId="${labPcrSample.sampleId}"
                                                   sampleName="${labPcrSample.sampleName}"
                                                   sampleType="${labPcrSample.sampleType}">
                                            <span class="custom-control-label">${labPcrSample.sampleName}</span>
                                        </label>
                                    </div>
                                </c:forEach>
                            </div>
...
<div class="row" style="padding: 20px 0">
    <div class="col-md-12">
        <button class="btn btn-yellow btn-lang center-block buildTask" data-toggle="popover" data-placement="top"
                data-content='扩增完成点击生成任务' data-trigger="focus">生成任务
        </button>
    </div>
</div>
            ...
            <div class="panel-body" style="display: none" id="GENERATION_TASK">
                <table class="table table-hover table-bordered bigTable">
                    ...
                    <tr>
                        ...
                        <td>${labPcrSampleListSize}</td>
                        <td>
                            <button class="btn btn-sm btn-blue-border inenterExperiment">进入实验</button>
                        </td>
                        <td>
                            <button type="button" name="deleteBtn" class="btn-icon btn-icon-red btn-icon-shanchu-red">
                                删除
                            </button>
                        </td>
                    </tr>
...
<div class="row btn-box">
    <div class="col-md-12" style="text-align: center">
        ...
        <%--<button class="btn btn-green btn-lang" type="button">生成记录表</button>--%>
    ...
    $(function () {
        ...
        $(".checkAll").click(function () {
            if ($("input[type='checkbox']:checked").length == $("input[type='checkbox']").length) {
                ...
                $("input[type='checkbox']").parents(".col-md-6").each(function () {
        ...
        function saveForm() {
            $.ajax({
                url: "<%=path%>/center/savePcrInfo?operateType=" + $("#operateType").val() + "&taskId=" + $("#taskId").val() + "&pcrEndDatetime=" + $("#pcrexperimentDate").val(),
                type: "post",
        ...
        function saveFormSample() {
            $.ajax({
                url: "<%=path%>/center/savePcrExperiment?operateType=" + $("#operateType").val() + "&taskId=" + $("#task").val() + "&pcrId=" + $("#pcrId").val(),
                type: "post",
                ...
                success: function (data) {
                    if (data.success || data.success == true || data.success == "true") {
                        ...
                        location.href = "<%=path%>/center/enterExperiment?taskId=" + $("#task").val() + "&taskStage=2";
                    } else {
                        alert("保存失败！");
                    }
                },
        ...
        function param() {
            ...
            sample.pcrPerson1 = $("#operateUser").text();
            return sample;
        }
        ...
        function paramSample() {
            ...
            for (var i = 0; i < sampleCnt; i++) {
                ...
                sample.samplePostion = $(".kongGreen").siblings("input[name='sampleId'][value='"+ sample.sampleId+"']").siblings("div").attr("value");
        ...
        if($("#many").val() == "1"){
            alert("请重新选板");
            location.href = "<%=path%>/center/pcrPuzzleExperiment?taskId=" + $("#taskId").val();
        }
        //通过板号获取数据
        $("#boardNo").change(function () {
            location.href = "<%=path%>/center/pcrPuzzleData?taskId=" + $("#taskId").val() + "&boardNo=" + $("#boardNo").val();

            /* $.ajax({
            <%--url: "<%=path%>/center/pcrPuzzleData?boardNo=" + $("#boardNo").val(),--%>
             url: "
            <%=path%>/center/pcrPuzzleData?boardNo=" + $("#boardNo").val(),
             type: "post",
             data: JSON.stringify(),
             contentType: "application/json; charset=utf-8",
             dataType: "json",
