...
public class WaitAcceptController extends BaseController {
    ...
    public ModelAndView waitAccept(HttpServletRequest request, LimsCaseInfoVo query,int visitNum, PageInfo pageInfo) {
        ...
        try {
            ...
            //案件状态_未受理
            query.setStatus(Constants.CASE_STATUS_PENDING);
            //查询委托单位（分局）
            ...
            //查询补送案件待受理数据
//            query.setAppendFlag(Constants.APPEND_FLAG_YES);
...
//            view.addObject("mainPageInfo",pageInfo.updatePageInfo(mainCaseCnt) );
    ...
    public ModelAndView caseManage(HttpServletRequest request, LimsCaseInfoVo query, PageInfo pageInfo) {
        ...
        try {
            ...
            //query.setStatus(Constants.CASE_STATUS_EXAMINING);
            if (query !=null) {
                query = resetCaseInfoQuery(query);
            }else {
                query = new LimsCaseInfoVo();
...
//                query.setOrderByClause("lci.ACCEPT_DATETIME desc");
            ...
            if(StringUtils.isNotBlank(query.getQueryFlag()) && query.getQueryFlag().equals(Constants.APPEND_FLAG_YES) && caseInfoList.size()>0){
       ...
       //view.setViewName("synthesize/caseShar");//跳转DNA数据库
        return view;
    }
    ...
    @SuppressWarnings("all")
    public ModelAndView databaseManage(HttpServletRequest request, LimsCaseInfoVo query, PageInfo pageInfo) {
        ModelAndView view = new ModelAndView();
        try {
...
//        view.setViewName("accept/caseManage");
        view.setViewName("synthesize/dataHouse");
        return view;
    }
     ...
     * @return
     */
    @RequestMapping("/fugitivesManage")
    @SuppressWarnings("all")
    public ModelAndView fugitivesManage(HttpServletRequest request, LimsCaseInfoVo query, PageInfo pageInfo) {
        ModelAndView view = new ModelAndView();
        try {
